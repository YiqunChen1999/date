...
"""
Utilities adapted from OneNet CrowdHuman tools.
...
def format_dt_json(tmp_dir: os.PathLike,
                   path2json: os.PathLike) -> os.PathLike:
    ...
    for res in json_results:
        if res['image_id'] in new_json_results:
            new_json_results[res['image_id']]['dtboxes'].append(
                {'box': res['bbox'], 'score': res['score']})
        else:
    ...
    path_to_odgt_file = osp.join(tmp_dir, 'predictions.odgt')
    with open(path_to_odgt_file, "w") as f:
        for db in new_json_results:
            line = json.dumps(new_json_results[db]) + '\n'
            f.write(line)
    return path_to_odgt_file


def format_gt_json(tmp_dir: os.PathLike,
                   path2json: os.PathLike) -> os.PathLike:
    ...
    for res in json_results['annotations']:
        if res['image_id'] in new_json_results:
            new_json_results[res['image_id']]['gtboxes'].append({
                'tag': 'person',
                'fbox': res['bbox'],
                # OneNet: here is wrong, but no matter
                'hbox': res['bbox_vis'],
            ...
            new_json_results[res['image_id']] = dict()
            new_json_results[res['image_id']]['ID'] = res['image_id']
            new_json_results[res['image_id']]['height'] = \
                img_dict[res['image_id']]['height']
            new_json_results[res['image_id']]['width'] = \
                img_dict[res['image_id']]['width']
    ...
    path_to_odgt_file = osp.join(tmp_dir, 'annotations.odgt')
    with open(path_to_odgt_file, "w") as f:
        for db in new_json_results:
            line = json.dumps(new_json_results[db]) + '\n'
            f.write(line)
...
class Image(object):
    ...
    def load(self, record, body_key, head_key, class_names, gtflag):
        ...
        if gtflag:
            ...
            body_bbox, head_bbox = self.load_gt_boxes(
                record, 'gtboxes', class_names)
            ...
            if self.eval_mode == 0:
                ...
                self.gtboxes = np.hstack((body_bbox[:, :-1],
                                          head_bbox[:, :-1],
                                          gt_tag.reshape(-1, 1)))
            else:
        ...
        if not gtflag:
            ...
            if self.eval_mode == 0:
                self.dtboxes = self.load_det_boxes(record,
                                                   'dtboxes',
                                                   body_key,
                                                   'score')
            elif self.eval_mode == 1:
                ...
                body_dtboxes = self.load_det_boxes(record,
                                                   'dtboxes',
                                                   body_key)
                head_dtboxes = self.load_det_boxes(record,
                                                   'dtboxes',
                                                   head_key,
                                                   'score')
    ...
    def clip_all_boader(self):
        ...
        if self.eval_mode == 2:
            ...
            self.dtboxes = _clip_boundary(self.dtboxes,
                                          self._height,
                                          self._width)
            self.gtboxes = _clip_boundary(self.gtboxes,
                                          self._height,
                                          self._width)
...
class Database(object):
    ...
    def loadData(self, fpath, body_key=None, head_key=None, if_gt=True):
        ...
        if if_gt:
            for record in records:
                ...
                self.images[record["ID"]].load(record,
                                               body_key,
                                               head_key,
                                               PERSON_CLASSES,
                                               True)
    ...
    def eval_MR(self, ref="CALTECH_-2"):
        ...
        if ref == "CALTECH_-2":
            # CALTECH_MRREF_2: anchor points (from 10^-2 to 1)
            # as in P.Dollar's paper
            ref = [0.0100, 0.0178, 0.03160, 0.0562, 0.1000,
                   0.1778, 0.3162, 0.5623, 1.000]
        else:
            # CALTECH_MRREF_4: anchor points (from 10^-4 to 1)
            # as in S.Zhang's paper
            ref = [0.0001, 0.0003, 0.00100, 0.0032, 0.0100,
                   0.0316, 0.1000, 0.3162, 1.000]
...
def evaluate_crowdhuman(gt_path: os.PathLike,
                        dt_path: os.PathLike,
                        target_key: str = "box",
                        mode: int = 0) -> Dict[str, float]:
    """
    Evaluate the coco results using COCOEval API.
    """
    print('Evaluating CrowdHuman...')
    database = Database(gt_path, dt_path, target_key, None, mode)
    database.compare()
    AP, recall, _ = database.eval_AP()
    mMR, _ = database.eval_MR()
    return dict(AP=AP, mMR=mMR, recall=recall)
